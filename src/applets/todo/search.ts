    ...
    constructor (applet: TodoApplet, query: string|Task = '') {
        ...
        const apply_edits = () => {
            if (bem_export_picker.path) {
                let content = '';
                for (const task of this.#filtered_tasks) content += task.text;
            ...
            if (bem_delete_checkbox.checked) {
                ...
                const del = (ast[0] as P.AstMeta).config;
                const add = (ast[1] as P.AstMeta).config;

                for (const task of this.#filtered_tasks) {
                    const conf = task.ast.config;

                    if (del.priority === conf.priority) delete conf.priority;
                    if (del.track === conf.track) delete conf.track;
                    if (del.due === conf.due) delete conf.due;
                    if (del.pin) delete conf.pin;
                    if (del.done) delete conf.done;
                    if (del.hide) delete conf.hide;
                    if (del.tags) for (const tag of del.tags) conf.tags?.delete(tag);

                    if (add.priority) conf.priority = add.priority;
                    if (add.track) conf.track = add.track;
                    if (add.due) conf.due = add.due;
                    if (add.pin) conf.pin = add.pin;
                    if (add.done) conf.done = add.done;
                    if (add.hide) conf.hide = add.hide;
